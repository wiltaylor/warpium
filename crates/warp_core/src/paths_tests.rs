...
fn test_data_dir_path() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(data_dir(), home_dir.join(".warpium"));
        } else if #[cfg(target_os = "linux")] {
            assert_eq!(data_dir(), home_dir.join(".local/share/warpium"));
        } else if #[cfg(windows)] {
            assert_eq!(data_dir(), home_dir.join("AppData\\Roaming\\warp\\Warpium\\data"));
        } else {
...
fn test_config_local_dir_path() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(config_local_dir(), home_dir.join(".warpium"));
        } else if #[cfg(target_os = "linux")] {
            assert_eq!(config_local_dir(), home_dir.join(".config/warpium"));
        } else if #[cfg(windows)] {
            assert_eq!(config_local_dir(), home_dir.join("AppData\\Local\\warp\\Warpium\\config"));
        } else {
            unimplemented!("Need to update tests for current platform!");
        }
    }
...
fn test_warp_home_config_dir_path() {
    ...
    let expected_dir_name = match ChannelState::data_profile() {
        Some(data_profile) => format!(".warpium-{data_profile}"),
        None => ".warpium".to_string(),
    };
...
fn test_cache_dir_path() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(cache_dir(), home_dir.join("Library/Application Support/dev.warp.Warpium"));
        } else if #[cfg(target_os = "linux")] {
            assert_eq!(cache_dir(), home_dir.join(".cache/warpium"));
        } else if #[cfg(windows)] {
            assert_eq!(cache_dir(), home_dir.join("AppData\\Local\\warp\\Warpium\\cache"));
        } else {
            unimplemented!("Need to update tests for current platform!");
        }
    }
...
fn test_state_dir_path() {
    ...
    cfg_if::cfg_if! {
        // ChannelState, by default, is configured for Channel::Oss.
        if #[cfg(target_os = "macos")] {
            assert_eq!(state_dir(), home_dir.join("Library/Application Support/dev.warp.Warpium"));
        } else if #[cfg(target_os = "linux")] {
            assert_eq!(state_dir(), home_dir.join(".local/state/warpium"));
        } else if #[cfg(windows)] {
            assert_eq!(state_dir(), home_dir.join("AppData\\Local\\warp\\Warpium\\data"));
        } else {
            unimplemented!("Need to update tests for current platform!");
        }
    }
...
fn test_project_path_for_oss_app_id() {
    let project_dirs = project_dirs_for_app_id(AppId::new("dev", "warp", "Warpium"), None)
        .expect("should be able to compute project dirs");
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(project_dirs.project_path(), "dev.warp.Warpium");
        } else if #[cfg(target_os = "linux")] {
            assert_eq!(project_dirs.project_path(), "warpium");
        } else if #[cfg(windows)] {
            assert_eq!(project_dirs.project_path(), "warp\\Warpium");
        } else {
            unimplemented!("Need to update tests for current platform!");
        }
    }
