...
impl AuthViewBody {
    ...
    fn render_force_login_disclaimer(
        ...
        let text = match self.variant {
            AuthViewVariant::RequireLoginCloseable  => {
                "In order to use Warpium’s AI features or collaborate with others, please create an account."
            }
            AuthViewVariant::HitDriveObjectLimitCloseable => {
    ...
    fn render_header(&self, appearance: &Appearance, ui_builder: &UiBuilder) -> Box<dyn Element> {
        ...
        let text = match self.variant {
            AuthViewVariant::Initial => "Welcome to Warpium!",
            AuthViewVariant::RequireLoginCloseable
            | AuthViewVariant::HitDriveObjectLimitCloseable
            | AuthViewVariant::ShareRequirementCloseable => "Sign up for Warpium",
        };
