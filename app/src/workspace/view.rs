...
// We use the product name in the Update Ready button to make it obvious which terminal is ready.
// This can lead to free advertising when users screen-share Warpium when an update is available.
const UPDATE_READY_TEXT: &str = "Update Warpium";
...
const VERSION_DEPRECATION_WITHOUT_PERMISSIONS_BANNER_TEXT: &str = "Some Warpium features may not work as expected without updating immediately, but Warpium is unable to perform the update.";
...
impl Workspace {
    ...
    fn unified_new_session_menu_items(
        ...
        if FeatureFlag::TabConfigs.is_enabled() {
            ...
            // 5. New tab config — V0: opens the TOML template.
            menu_items.push(
    ...
    /// a user clicks "Update Warpium" in the top right of the tab bar.
            ...
            if let Some(version) = ChannelState::app_version() {
                ...
                match autoupdate::get_update_state(ctx) {
                    ...
                    AutoupdateStage::UnableToUpdateToNewVersion { .. } => menu_items.push(
                        MenuItemFields::new("Update Warpium manually")
    ...
    fn user_menu_items(&self, app: &AppContext) -> Vec<MenuItem<WorkspaceAction>> {
            ...
            && ChannelState::show_autoupdate_menu_items()
        {
            match autoupdate::get_update_state(app) {
                ...
                {
                    items.push(
                        MenuItemFields::new("Update and relaunch Warpium")
                            .with_on_select_action(WorkspaceAction::ApplyUpdate)
        ...
        #[cfg(not(target_family = "wasm"))]
        items.push(
            MenuItemFields::new("View Warpium logs")
            ...
            ctx.request_desktop_notification_permissions(move |view, outcome, ctx| {
                match &outcome {
                    ...
                    RequestPermissionsOutcome::PermissionsDenied => {
                        ...
                        view.toast_stack.update(ctx, |toast_stack, ctx| {
                            let toast = DismissibleToast::error(
                                "Warpium doesn't have permission to send desktop notifications.".to_string(),
                            )
                ...
                if let Some(version) = ChannelState::app_version() {
                    ...
                    if FeatureFlag::AvatarInTabBar.is_enabled() {
                        self.update_toast_stack.update(ctx, |stack, ctx| {
                            ...
                            let toast = DismissibleToast::default(String::from("Warpium updated!"))
                                .with_link(link);
            ...
            match autoupdate::get_update_state(app) {
                    ...
                    if !self.autoupdate_unable_to_update_banner_dismissed =>
                {
                    ...
                    Some(WorkspaceBannerFields {
                        ...
                        secondary_button: None,
                        button: Some(WorkspaceBannerButtonDetails {
                            text: "Update Warpium manually".to_string(),
                    ...
                    if !self.autoupdate_unable_to_launch_new_version =>
                {
                    let description =
                        if is_incoming_version_past_current(new_version.soft_cutoff.as_deref()) {
                            ...
                            "Warpium was unable to launch the new installed version.".to_owned()
                        };
    ...
    fn open_link_on_desktop(&mut self, url: &Url, ctx: &mut ViewContext<Self>) {
        ...
        if !is_app_installed {
            ...
            let toast_message = format!(
                "Have Warpium installed but redirecting to download page?\nEnable Local Network Access for {} in your browser.",
                ChannelState::server_root_url()
            );
            self.toast_stack.update(ctx, |toast_stack, ctx| {
